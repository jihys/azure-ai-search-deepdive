...
PDF/PPTX (로컬 data/raw/ 에서 수동 업로드)
  ...
  → Pipeline B-1 (Basic PDF):           multimodal-basic-indexer-pdf
      DI Layout → Custom markdown_split → Embedding → multimodal-basic-index-pdf
  → Pipeline B-2 (Basic PPTX):          multimodal-basic-indexer-pptx
      DI Layout → Custom pptx_page_split → Embedding → multimodal-basic-index-pptx
  → Pipeline B-3 (Verbalized PDF):      multimodal-verbalized-indexer-pdf
      DI Layout → Custom verbalize (GPT-5.4 Vision) → Custom markdown_split → Embedding → multimodal-verbalized-index-pdf
  → Pipeline B-4 (Verbalized PPTX):     multimodal-verbalized-indexer-pptx
      DI Layout → Custom verbalize (GPT-5.4 Vision) → Custom markdown_split → Embedding → multimodal-verbalized-index-pptx
...
```
[비교 1: PDF 인덱싱 전략]
  B-1 Basic PDF (DI Layout + markdown_split)
  vs B-3 Verbalized PDF (DI Layout + GPT verbalize + markdown_split)
  vs B-5 Content Understanding PDF (CU Skill 단일, semantic chunking + image desc)

[비교 2: PPTX 인덱싱 전략]
  B-2 Basic PPTX (DI Layout + pptx_page_split)
  vs B-4 Verbalized PPTX (DI Layout + GPT verbalize + markdown_split)
...
| Q9 | `func-skills` = 시나리오 B 전용 | AI Search Skillset이 인덱싱 중 호출하는 Custom Web API Skill. 시나리오 A (법률)에서는 미사용 |
...
| Q14 | Verbalized PPTX split = `markdown_split` | verbalize 출력이 이미 텍스트화된 Markdown이므로 pptx_page_split이 아닌 markdown_split 사용 |
