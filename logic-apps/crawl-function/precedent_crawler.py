...
def _to_iso_date(korean_date: str) -> str:
    """한국어 날짜 → ISO 8601 변환.

    '2024. 1. 15.' → '2024-01-15T00:00:00Z'
    '2024.01.15'   → '2024-01-15T00:00:00Z'
    변환 불가 시 빈 문자열 반환.
    """
    if not korean_date or not korean_date.strip():
        return ""
    m = re.match(r"(\d{4})\.?\s*(\d{1,2})\.?\s*(\d{1,2})\.?", korean_date.strip())
    if m:
