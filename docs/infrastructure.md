...
| **Stage 1** | Logic Apps (Crawl + Data Preprocessing) | Blob 업로드만 |
| **Stage 2** | AI Search Native Skillset | Built-in Skillset 비교 (Basic vs Verbalized) |
| **인덱스** | 4개 (prec/detc/expc/admrul) | 2개 per source (basic/verbalized) |
| **노트북** | 02 → 03(Indexing) → 04(Search) → 05 | 06 |
...
5. [파이프라인 구성 (Logic Apps 워크플로우 오케스트레이션)](#5-파이프라인-구성-logic-apps-워크플로우-오케스트레이션)
6. [인덱싱 파이프라인 (AI Search Skills — 2개 시나리오, 6개 인덱스)](#6-인덱싱-파이프라인-ai-search-skills)
...
| 13 | Function App | `func-crawl-ragi-<suffix>` | Python 3.11 / Linux | 법령 크롤러 (law.go.kr 수집) |
...
| 8 | Function App | `func-crawl-ragi-<suffix>` | Korea Central | EP1 | 법령 크롤러 (law.go.kr 수집) |
...
**워크플로우 흐름 (crawl-preprocess-workflow):**

```
[Recurrence 트리거 - 매일 21:00 UTC (06:00 KST)]
    │
    ▼
═══════════════════════════════════════════════════════════════
║ Phase 1: CRAWLING (모든 소스 병렬)                           ║
═══════════════════════════════════════════════════════════════
    │
    ├─→ [Crawl Function] /api/crawl
    │   body: { "source": "all" }
    │   ├─ prec (판례) - 병렬 수집
    │   ├─ detc (헌재 판례) - 병렬 수집
    │   ├─ expc (입법예고) - 병렬 수집
    │   └─ admrul (행정심판) - 병렬 수집
    │   → raw-documents/{source}/{date}/ 저장
    │
    ▼
[Crawl Success?]
    │
    ...
    ▼
═══════════════════════════════════════════════════════════════
║ Phase 2: PREPROCESSING - DATA PREPROCESSING (각 소스 병렬)    ║
═══════════════════════════════════════════════════════════════
    │
    ├─→ [Preprocess Function] /api/preprocess (prec)
    │   body: { "source": "prec", "crawl_date": "2026-04-19" }
    │   → 메타데이터 정규화 + 포맷 통일
    │   → processed-documents/prec/{date}/ 저장
    │
    ├─→ [Preprocess Function] /api/preprocess (detc)
    │
    ├─→ [Preprocess Function] /api/preprocess (expc)
    │
    └─→ [Preprocess Function] /api/preprocess (admrul)
    │
    ▼
[All Preprocess Success?]
    │
    ├─ YES →
    │
    ▼
═══════════════════════════════════════════════════════════════
║ Phase 3: INDEXING (AI Search Indexer - 별도 스케줄)         ║
═══════════════════════════════════════════════════════════════
    │ (선택적: 별도 스케줄로 실행 또는 즉시 트리거)
    │
    ├─→ Run_Search_Indexer
    │   POST /indexers/{indexer-name}/run
    │   → processed-documents/{source}/{date}/ 읽기
...
**주요 특징:**
- **Phase 1 (크롤링)**: 4개 소스를 Function 내부에서 병렬 처리
- **Phase 2 (Data Preprocessing)**: 4개 소스를 Logic App에서 병렬 호출 (독립적 함수)
- **Phase 3 (인덱싱)**: AI Search Indexer가 증분 인덱싱 (별도 스케줄)
- **에러 처리**: 각 단계별 실패 시 알림 (SendGrid 이메일)
- **초기 빌드**: 모든 raw 데이터를 한 번에 Data Preprocessing 처리 (이후 증분)

> **크롤링 vs Data Preprocessing vs 인덱싱 분리 이유:**
> - 크롤링은 web I/O 의존 (1-2시간 소요)
> - Data Preprocessing은 메타데이터 정규화/포맷 통합 처리 (수분 소요, 병렬 최적화)
> - 인덱싱은 검색 쿼리 영향 최소화 (별도 스케줄)
...
## 5. 파이프라인 구성 (Logic Apps 워크플로우 오케스트레이션)

기본 1개의 Logic Apps 워크플로우로 전체 데이터 처리 오케스트레이션:

### A. crawl-preprocess-workflow (메인 오케스트레이션, 매일 21:00 UTC (06:00 KST))

```
[Logic App - crawl-preprocess-workflow]
Recurrence: 매일 21:00 UTC (06:00 KST)
    │
    │ Phase 1: CRAWLING (HTTP POST)
    ├─ func-crawl-ragi /api/crawl (source=all)
    │  → law.go.kr 모든 소스 병렬 크롤링
    │  → Blob: raw-documents/{source}/{date}/
    │
    │ Phase 2: DATA PREPROCESSING (4개 소스 병렬, HTTP POST)
    ├─ func-preprocess-ragi /api/preprocess (source=prec)
    ├─ func-preprocess-ragi /api/preprocess (source=detc)
    ├─ func-preprocess-ragi /api/preprocess (source=expc)
    ├─ func-preprocess-ragi /api/preprocess (source=admrul)
    │  → 각 소스별 독립적 Data Preprocessing 처리
    │  → Blob: processed-documents/{source}/{date}/
    │
    │ Phase 3: RESULT (수집)
    └─ 모든 결과 수집 + alert (실패 시)
```

### B. crawl-workflow (레거시/디버그용 — 기본 미사용)

```
[Logic App - crawl-workflow]
...
### C. rag-indexing-workflow (레거시/디버그용 — 기본 미사용)

```
[Logic App - rag-indexing-workflow]
Trigger: Blob Storage (processed-documents/)
    │
    ├─ Document Intelligence Layout
    └─ OpenAI Embedding
        → (과거 방식) AI Search Index 자동 인덱싱
```

**중요 구분:**
- **crawl-preprocess-workflow**: 크롤링 + Data Preprocessing + AI Search 증분 인덱싱 준비 (전체 오케스트레이션)
- **crawl-workflow**: 레거시 디버그용(기본 배포/운영에서는 제외)
- **rag-indexing-workflow**: 레거시 디버그용(기본 배포/운영에서는 제외)
- **Function Apps**:
  - `func-crawl-ragi`: 크롤링 실행
    - `func-preprocess-ragi`: Data Preprocessing (메타데이터 정규화/포맷 통일, 4개 소스 독립 실행)
- **AI Search Indexer**: processed-documents로부터 벡터 임베딩 + 인덱싱

---

## 6. 인덱싱 파이프라인 (AI Search Skills — 2개 시나리오)

모든 파이프라인은 **AI Search Indexer가 내부에서 실행**합니다. Split/Embedding/Ingest 전부 AI Search가 담당합니다.

---

### 시나리오 A: 법령 텍스트 인덱싱 (4개 인덱스)

> **소스**: `processed-documents/{source}/{date}/` (Logic Apps Stage 1이 생성)
...
**공통 Skillset (Native Only)**:
```
processed-documents/{source}/{date}/*.json
    │  AI Search Indexer (매일 별도 스케줄, 증분)
    │  ← Shared Private Link (spl-blob)
    ▼
[Skill 1] #Microsoft.Skills.Text.SplitSkill
    ├─ textSplitMode: "pages"
    ├─ maximumPageLength: 2000
    ├─ pageOverlapLength: 200
    └─ 출력: pages[]
    ▼
[Skill 2] #Microsoft.Skills.Text.AzureOpenAIEmbeddingSkill
...
### 시나리오 B: 멀티모달 PDF/PPTX 인덱싱 (Built-in Skill 비교 데모)

> **소스**: `raw/pdf/{source}/` (수동 업로드)  
> **목적**: Basic (DI Layout) vs Verbalized (GenAI Prompt) Skillset 검색 품질 비교

#### Pipeline B-1/B-2: Basic (Function App 불필요)

```
raw/pdf/{source}/*.pdf (또는 *.pptx)
    │  AI Search Indexer ← Shared Private Link (spl-blob, spl-docintel)
    ▼
[Skill 1] #Microsoft.Skills.Util.DocumentIntelligenceLayoutSkill
    ├─ outputMode: oneToOne
    ├─ prebuilt-layout → Markdown 변환
    └─ 출력: layout_markdown
    ▼
[Skill 2] #Microsoft.Skills.Text.SplitSkill
    ├─ textSplitMode: "pages"
    ├─ maximumPageLength: 2000, pageOverlapLength: 200
    └─ 출력: pages[]
    ▼
[Skill 3] #Microsoft.Skills.Text.AzureOpenAIEmbeddingSkill (3072D)
    ▼
{source}-multimodal-basic-index
```

#### Pipeline B-3/B-4: Verbalized (Function App 불필요 — Built-in GenAI Prompt Skill)

```
raw/pdf/{source}/*.pdf (또는 *.pptx)
    │  AI Search Indexer ← Shared Private Link (spl-blob, spl-aiservices)
    │  imageAction: generateNormalizedImages
    ▼
[Skill 1] #Microsoft.Skills.Custom.ChatCompletionSkill  ← Built-in GenAI Prompt
    ├─ GPT-5.4 Vision: normalized_images → 풍부한 자연어 설명 생성
    └─ 출력: description (per image)
    ▼
[Skill 2] #Microsoft.Skills.Text.MergeSkill
    ├─ 원본 텍스트 + 이미지 설명 inline 병합
    └─ 출력: merged_content
    ▼
[Skill 3] #Microsoft.Skills.Text.SplitSkill (pages mode)
    ▼
[Skill 4] #Microsoft.Skills.Text.AzureOpenAIEmbeddingSkill (3072D)
    ▼
{source}-multimodal-verbalized-index
```

#### Basic vs Verbalized 비교

| 항목 | Basic (DI Layout Only) | Verbalized (GenAI Prompt Skill) |
|------|--------------------------|----------------------|
| 이미지 이해 | ❌ 텍스트·표만 추출 (이미지 무시) | ✅ GPT-5.4 Vision 이미지 상세 설명 |
| 도면 이해 | ❌ 구성요소 관계 파악 불가 | ✅ 전기 심볼, 계통 흐름, 사양 해석 |
| RAG 인용 | △ 텍스트·표 기반만 가능 | ✅ 이미지 설명을 컨텍스트로 제공 |
| 추가 비용 | 없음 | GPT-5.4 Vision 호출 비용 (Built-in) |
| Function App | 불필요 (Built-in Skill만 사용) | 불필요 (Built-in Skill만 사용) |
...
6. Private Endpoints (모든 서비스 ID 참조)
   │
7. [코드 배포] 2개 Function App 각각에 올바른 소스 배포 (섹션 9)
   │
...
Bicep으로 Function App 인프라를 배포한 후, 2개 Function App에 각각 올바른 소스 코드를 배포합니다.
...
> ℹ️ `func-skills-ragi` (Custom WebAPI Skill)는 Built-in Skill 전환으로 **미사용**합니다. `skills-function/` 디렉토리는 참고용으로 보존됩니다.
...
for app in func-crawl-cons-ragi-<suffix> func-preprocess-ragi-<suffix>; do
  echo "=== $app ==="
