...
| **Stage 2** | AI Search Native Skillset | Native vs Custom+Native Skillset 비교 |
| **인덱스** | 4개 (prec/detc/expc/admrul) | 2개 (native/custom) |
...
## 6. 인덱싱 파이프라인 (AI Search Skills — 2개 시나리오, 6개 인덱스)
...
### 시나리오 B: 멀티모달 PDF/PPTX 인덱싱 (2개 인덱스 — 비교 데모)

> **소스**: `raw/pdf/{source}/` (수동 업로드)  
> **목적**: Native Skillset vs Custom+Native Skillset 검색 품질 비교

#### Pipeline B-1: Native-only (Function App 불필요)
...
#### Pipeline B-2: Custom+Native (Function App 필요 — GPT-5.4 Verbalization)
...
[Skill 1] #Microsoft.Skills.Util.DocumentIntelligenceLayoutSkill
    └─ 출력: markdown_document
    ▼
[Skill 2] #Microsoft.Skills.Custom.WebApiSkill  ← func-crawl-ragi /api/verbalize
    ├─ GPT-5.4 Vision: 이미지/도표 → 풍부한 자연어 설명 생성
    └─ 출력: verbalized_markdown
    ▼
[Skill 3] #Microsoft.Skills.Text.SplitSkill (markdown mode)
    ▼
...
#### Native 스킬 vs GPT-5.4 Verbalization 비교

| 항목 | Native ImageAnalysisSkill | GPT-5.4 Verbalization |
|------|--------------------------|----------------------|
| 설명 수준 | "a chart showing data" (1~2문장) | 구조·관계·수치 포함 상세 설명 |
| 도면 이해 | ❌ 구성요소 관계 파악 불가 | ✅ 전기 심볼, 계통 흐름, 사양 해석 |
| RAG 인용 | ❌ 의미 있는 텍스트 없음 | ✅ 상세 설명을 컨텍스트로 제공 |
| 추가 비용 | 없음 | GPT-5.4 Vision 호출 비용 |
| Function App | 불필요 | 필요 |
