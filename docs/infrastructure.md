...
| **Stage 1** | Logic Apps (Crawl + Data Integration) | Blob 업로드만 |
...
[Crawl Success?]
    ...
    ▼
═══════════════════════════════════════════════════════════════
║ Phase 2: PREPROCESSING - DATA INTEGRATION (각 소스 병렬)    ║
═══════════════════════════════════════════════════════════════
    │
...
- **Phase 2 (Data Integration)**: 4개 소스를 Logic App에서 병렬 호출 (독립적 함수)
- **Phase 3 (인덱싱)**: AI Search Indexer가 증분 인덱싱 (별도 스케줄)
- **에러 처리**: 각 단계별 실패 시 알림 (SendGrid 이메일)
- **초기 빌드**: 모든 raw 데이터를 한 번에 Data Integration 처리 (이후 증분)

> **크롤링 vs Data Integration vs 인덱싱 분리 이유:**
> - 크롤링은 web I/O 의존 (1-2시간 소요)
> - Data Integration은 메타데이터 정규화/포맷 통합 처리 (수분 소요, 병렬 최적화)
...
### A. crawl-preprocess-workflow (메인 오케스트레이션, 매일 06:00 KST)

```
[Logic App - crawl-preprocess-workflow]
Recurrence: 매일 06:00 KST (= 21:00 UTC 이전날)
    │
    ...
    │
    │ Phase 2: DATA INTEGRATION (4개 소스 병렬, HTTP POST)
...
**중요 구분:**
- **crawl-preprocess-workflow**: 크롤링 + Data Integration + AI Search 증분 인덱싱 준비 (전체 오케스트레이션)
- **crawl-workflow**: 레거시 디버그용(기본 배포/운영에서는 제외)
- **rag-indexing-workflow**: 레거시 디버그용(기본 배포/운영에서는 제외)
- **Function Apps**:
  - `func-crawl-ragi`: 크롤링 실행
    - `func-preprocess-ragi`: Data Integration (메타데이터 정규화/포맷 통일, 4개 소스 독립 실행)
